    ...
    Func: Fn(F1) -> F2,
{
    ...
    let h: F1 = (upper_limit - lower_limit)
        / F1::from(n).expect("failed to convert n to float");
    ...
    Func: Fn(F1) -> F2,
{
    ...
    let interior: f64 = (1..n)
        .map(|i| func(lower_limit + F1::from(i).unwrap() * h).to_f64().unwrap())
        .sum();
    ...
    let first = func(lower_limit).to_f64().unwrap()
        + 3.0
            * (func(lower_limit + h3).to_f64().unwrap()
                + func(lower_limit + F1::from(2).unwrap() * h3).to_f64().unwrap());
